...
First, you need to read and understand this article: [How to connect my data](Connecting-Data.md)
...
It is called `no_data` for [UDF](UDF.md#bars) and `noData` for [JS API](JS-Api.md#getbarssymbolinfo-resolution-periodparams-onhistorycallback-onerrorcallback)
...
Please read [Symbology](Symbology.md) thoroughly. The number of decimal places is calculated based on `minmov` and `pricescale` values.
